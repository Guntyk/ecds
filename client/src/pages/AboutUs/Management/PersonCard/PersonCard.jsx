...
export const PersonCard = ({
  person: { name, surname, role, socials, biography, photo },
}) => {
      ...
      <div className={cn(styles.card, { [styles.open]: isOpen })}>
        ...
        >
          <ImageComponent
            ...
            src={
              `${process.env.REACT_APP_BASE_API_URL}${photo?.url}` ||
              "https://placehold.co/234"
            }
                ...
                {socials.map(({ id, url }) => {
                  ...
                  if (socialName) {
                    return (
                      <li key={id}>
                        <Link
                          className={cn(styles.social, styles[socialName])}
                          path={url}
                          external
                        />
                      </li>
